...
따라서 Project Service Automation(PSA)에서 사용할 수 있는 가격 차원 값에는 두 가지 타입이 있습니다: 
...
- [가격 책정 차원으로 맞춤 필드 설정](set-up-pricing-dimensions.md)
...
또한 가격 책정 차원이 표여야 하는지 옵션 집합이어야 하는지도 고려해야 합니다. 10 또는 12를 초과하는 차원값의 변경을 예측하고 이러한 값에 추가 속성이 필요한 경우 옵션 집합보다는 엔터티를 만들 수 있습니다. 값 추가 또는 제거와 같은 옵션 집합을 유지하려면 관리자 또는 개발자가 요구되지만 표에 새 행을 추가하는 것은 대부분의 사용자가 할 수 있습니다.
